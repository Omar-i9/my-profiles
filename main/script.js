document.addEventListener('DOMContentLoaded', () => {
  const STORAGE = {
    ...
    ayahX: 'omarp_ayah_x',
    ayahY: 'omarp_ayah_y',
    ayahMode: 'omarp_ayah_mode',
    ayahPage: 'omarp_ayah_page',
  };

  const AUDIO_BASE = './audio/';
  const IMAGE_BASES = ['./images/', './photo/'];
  ...
  let ayahNum = document.getElementById('ayahNum');
  let ayahText = document.getElementById('ayahText');
  let ayahRef = document.getElementById('ayahRef');
  let ayahNote = document.getElementById('ayahNote');
  let ayahDetail = document.getElementById('ayahDetail');
  ...
  let ayahTimer = null;
  let ayahDelay = 5000;
  let ayahMode = getString(STORAGE.ayahMode, 'compact');
  let ayahPage = getNumber(STORAGE.ayahPage, 0);
  let ayahExpandedIndex = null;
  let ayahOrder = [];
  let ayahVisible = getBoolean(STORAGE.ayahVisible, true);
  let ayahDragState = { active: false, started: false, offsetX: 0, offsetY: 0, startX: 0, startY: 0 };
...
function initAyahSystem() {
  ...
  function bindEvents() {
    musicFab?.addEventListener('click', () => { musicSheet.classList.add('open'); overlay.classList.add('open'); });
    closeMusicBtn?.addEventListener('click', () => { musicSheet.classList.remove('open'); overlay.classList.remove('open'); });
    overlay?.addEventListener('click', () => { musicSheet.classList.remove('open'); overlay.classList.remove('open'); closeTrackMenu(); closeMoreSocials(); });
    ...
    seekBar?.addEventListener('pointerdown', () => { isSeeking = true; showSeekTooltip(); });
    seekBar?.addEventListener('pointerup', () => { isSeeking = false; hideSeekTooltipSoon(); });
    seekBar?.addEventListener('pointercancel', () => { isSeeking = false; hideSeekTooltipSoon(); });
    ...
    seekWrap?.addEventListener('touchstart', () => { if (audio.duration) showSeekTooltip(); }, { passive: true });
    ...
    copyButtons.forEach((btn) => btn.addEventListener('click', () => copyText(btn.dataset.copy)));
